...
use ai_request::AiRequest;
use game_state::GameState;
use game_prompt::GamePrompt;
...
impl GameEngine {
    fn new(api_key: String) -> Self {
        GameEngine {
            ai_client: AiRequest::new(api_key),
            game_state: None,
        }
    }

    async fn start_game(&mut self) -> Result<(), Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let api_key = std::fs::read_to_string("api.key").expect("Failed to read API key from file");
    let mut game = GameEngine::new(api_key);
